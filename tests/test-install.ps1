...
try {
 ...
 $r += Assert-JsonField ".agentx/version.json" "version" "5.2.6" "version=5.2.6"
...
try {
 ...
 $r += Assert-JsonField ".agentx/version.json" "version" "5.2.6" "force: version restored"
 foreach ($t in $TEMP_FILES) { $r += Assert-PathNotExists $t "no-temp: $t" }
...
foreach ($t in $global:TestResults) {
 ...
 $totalFail += $t.Fail
 $totalTests += $t.Total
 $icon = if ($t.Fail -eq 0) { "[PASS]" } else { "[FAIL]" }
